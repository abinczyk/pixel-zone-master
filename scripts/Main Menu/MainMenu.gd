...
func _on_Play_pressed():
	...
	if a.file_exists("user://dlc_3/stage.tscn"):
		$TextureRect/WindowDialog2.popup_centered()
		return
...
func _on_Continue_pressed():
	...
	if con.file_exists("user://1.save") and con.file_exists("user://2.save") and not con.file_exists("user://3.save") and not con.file_exists("user://4.save"):
		background_load.load_scene("scenes/stages/ice_cap_adventure/ice_01.tscn")
	if con.file_exists("user://1.save") and con.file_exists("user://2.save") and con.file_exists("user://3.save") and not con.file_exists("user://4.save"):
		background_load.load_scene("scenes/stages/ice_cap_adventure/ice_02.tscn")
	if con.file_exists("user://1.save") and con.file_exists("user://2.save") and con.file_exists("user://3.save") and con.file_exists("user://4.save"):
